...
export function matchDropdownOption(rawStr, optionsArray) {
  ...
  for (const opt of optionsArray) {
    ...
    if (cleanedRaw === cleanedOpt || cleanedRaw.includes(cleanedOpt) || cleanedOpt.includes(cleanedRaw)) {
      return opt; // Retorna el valor EXACTO del select (con tildes, mayúsculas/minúsculas)
    }
  }
  
  return rawStr; // Si no hay coincidencia, retorna lo que había (para el fallback de input libre)
}
